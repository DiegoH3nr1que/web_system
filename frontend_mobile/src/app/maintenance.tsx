  ...
  export default function HistoricoManutencao() {
    ...
    const marcarEmManutencao = (id: number) => {
      setManutencoes((prev) =>
        prev.map((item) =>
          item.id === id
            ? { ...item, status: "Em Andamento", comentario }
            : item
        )
      );
    ...
    return (
      <ScrollView contentContainerStyle={{ flexGrow: 1 }} className="p-4 my-6">
          ...
          renderItem={({ item }) => (
            <View className="my-4 p-4 border rounded-lg bg-white shadow-md">
              <Image
                ...
                borderRadius={10}
              />
              <Text className="text-xl font-bold text-gray-800">{item.descricao}</Text>
